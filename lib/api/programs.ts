...
function mapLesson(l: any): ProgramLesson {
  return {
    ...
    contentUrl: l.video_url || l.file_url || l.content_url || null,
    videoUrl: l.video_url || null,
    fileUrl: l.file_url || null,
    fileType: l.file_type || null,
    durationMinutes: l.estimated_duration_minutes ?? l.duration_minutes ?? undefined,
    orderIndex: l.position ?? l.order_index ?? 0,
...
function mapModule(m: any): ProgramModule {
  return {
    ...
    contentUrl: m.video_url || m.file_url || m.content_url || null,
    videoUrl: m.video_url || null,
    fileUrl: m.file_url || null,
    fileType: m.file_type || null,
    durationMinutes: m.estimated_duration_minutes ?? m.duration_minutes ?? undefined,
    orderIndex: m.position ?? m.order_index ?? 0,
    isRequired: Boolean(m.is_required),
    requiresQuizPass: Boolean(m.requires_quiz_pass),
    quiz: m.quiz ? mapQuiz(m.quiz) : m.quizzes?.[0] ? mapQuiz(m.quizzes[0]) : undefined,
      ...
      data: programsRaw.map((prog: any) => {
        ...
        return {
          ...
          program: {
            ...
            durationMinutes: prog.duration_minutes ?? (prog.estimated_duration_hours ? prog.estimated_duration_hours * 60 : 0),
            estimatedDurationHours: prog.estimated_duration_hours,
            totalContent: prog.total_modules || prog.modules?.length || 0,
            learningObjectives: prog.learning_objectives,
            difficultyLevel: prog.difficulty_level,
...
export async function getProgram(programId: string): Promise<{
  ...
  return {
    program: {
      ...
      imageUrl: d.program.image_url || d.program.thumbnail_url,
      durationMinutes: d.program.duration_minutes || (d.program.estimated_duration_hours ? d.program.estimated_duration_hours * 60 : undefined),
      estimatedDurationHours: d.program.estimated_duration_hours,
      totalContent: d.program.total_content || d.program.total_modules || d.program.modules?.length || 0,
      learningObjectives: d.program.learning_objectives,
      difficultyLevel: d.program.difficulty_level,
